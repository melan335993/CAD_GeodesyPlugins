...
namespace mavCAD
{
    public class ST_Deviation : FastCAD, IExtensionApplication
    {
        ...
        public void MyCommand()
        {
            ...
            window.textbox_deviationRedAngle.Text = "0.015";

            double toleranceRED = window._data.deviationRed;
            double toleranceREDAngle = window._data.deviationRedAngle;
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction()) // запускаем транзакцию
            {
                ...
                try
                {
                    ...
                    {
                        MyDeviationST(blockScale, toleranceRED, toleranceREDAngle, "Плановые отклонения [В] [Н]", toleranceCreate, layerPoly, layerPoints, @"C:\\arrowDinoBlocks.dwg");
                        ed.Regen();
                        tr.Commit();
                    }
